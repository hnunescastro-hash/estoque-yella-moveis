...
# estoque-yella-chave-equipe (código dos vendedores), estoque-yella-deploy-key e
# estoque-yella-openrouter (juiz do cruzamento Matina x Igaporã).
set -eu
...
gcloud run deploy "$SERVICO" --source "$PASTA" \
  ...
  --set-secrets "CHAVE_ADMIN=estoque-yella-chave-admin:latest,CHAVE_EQUIPE=estoque-yella-chave-equipe:latest,DEPLOY_KEY=estoque-yella-deploy-key:latest,OPENROUTER_API_KEY=estoque-yella-openrouter:latest" \
